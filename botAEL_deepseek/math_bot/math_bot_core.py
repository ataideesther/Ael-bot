...
class MathBot:
    ...
    def _limpar_pensamento(self, texto):
        ...
        if not isinstance(texto, str):
            return texto
        # Remove quaisquer blocos <think>...</think> (case-insensitive)
        texto_sem_think = re.sub(r"<think>[\s\S]*?</think>", "", texto, flags=re.IGNORECASE)
        # Se a tag de fechamento não vier, remove tudo a partir de <think>
    ...
    def iniciar_conversa(self):
        ...
        return self._limpar_pensamento(resposta_prof)
